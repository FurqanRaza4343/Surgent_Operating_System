...
// Same graceful-degradation pattern used across the app (Navbar,
// ClaimPlanPage, PlanContext) when VITE_CLERK_PUBLISHABLE_KEY isn't set.
function ClerkNotConfigured() {
  return (
    <div className="rounded-3xl border border-sand-200 bg-white p-8 text-center shadow-lift">
      <h1 className="font-display text-2xl font-600 text-ink">Sign-in isn&apos;t configured yet</h1>
      <p className="mt-2 text-sm text-ink-muted">
        Set <code className="rounded bg-sand-100 px-1.5 py-0.5 text-xs">VITE_CLERK_PUBLISHABLE_KEY</code> to enable
        authentication, or explore the product first.
      </p>
      <Link
        to="/demo"
