...
// Owner-only — the shareable link a doctor uses to self-register (see
// app/auth/DoctorApplyPage.tsx and backend/src/services/practice/
// practice_services.py's doctor_signup_code). Regenerating invalidates any
// previously shared link.
export function DoctorSignupLinkCard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const result = await getDoctorSignupCode(authedFetch);
  ...
  async function handleRegenerate() {
    ...
    setBusy(true);
    try {
      const result = await regenerateDoctorSignupCode(authedFetch);
  ...
  return (
    <div className="mb-6 rounded-3xl border border-sand-200 bg-white p-5 shadow-[0_4px_20px_rgba(11,29,38,0.05)]">
      <div className="flex items-center gap-2 text-sm font-bold text-ink">
        <Link2Icon className="h-4 w-4 text-teal-600" /> Doctor signup link
      </div>
      <p className="mt-1 text-xs text-ink-muted">
        Share this with a doctor to let them register themselves — you&apos;ll review and approve their access before
        they get dashboard access.
      </p>
